...
class E2eePolicy
  ...
  INVALID_DEVICE_CODE = "e2ee_invalid_sender_device"
  ...
  class << self
    ...
    def validate_sender_device(user:, payload:, request: nil)
      sender_device_id = device_id_from(request:, params: payload)
      payload_hash = payload.respond_to?(:to_h) ? payload.to_h.with_indifferent_access : {}.with_indifferent_access
      sender_fingerprint = payload_hash[:sender_key_fingerprint]

      unless valid_device_id?(sender_device_id) && sender_fingerprint.present?
        return invalid_sender_device_error
      end

      device = user&.user_e2ee_keys&.find_by(device_id: sender_device_id, revoked_at: nil)
      unless device && secure_equal?(device.key_fingerprint.to_s, sender_fingerprint.to_s)
        return invalid_sender_device_error
      end
    ...
    def bundle_fingerprint(encryption_public_key, signing_public_key)
      encryption_public_key = canonical_public_key(encryption_public_key)
      signing_public_key = canonical_public_key(signing_public_key)
      return nil if encryption_public_key.blank? || signing_public_key.blank?

      Digest::SHA256.hexdigest("#{encryption_public_key}|#{signing_public_key}")
    end

    def canonical_public_key(value)
      return nil if value.blank?

      decoded = Base64.strict_decode64(value.to_s)
      return nil unless decoded.bytesize == PUBLIC_KEY_BYTES
    ...
    def invalid_sender_device_error
      {
        message: "E2EE sender device metadata does not match an active device key for this user",
