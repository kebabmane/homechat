  ...
  class ResponderJob < ApplicationJob
    ...
    def perform(bot_id, message_id, reason = "mention")
      ...
      begin
        ...
        Rails.logger.debug("Failed to update presence for bot_identity_id=#{identity_user.id}: #{e.class}")
      end
      ...
      Rails.logger.error("Bot responder failed for bot ##{bot_id}: #{e.class}")
    ...
    def ensure_membership(channel, user)
      ...
      Rails.logger.info("Bot identity id=#{user.id} not invited to private channel_id=#{channel.id}; skipping")
      false
    end
    ...
    def generate_response(bot, identity_user, channel, message, reason)
      ...
      if model.blank?
      Rails.logger.warn("Bot id=#{bot.id} has no model configured")
        return nil
      end
      ...
      Rails.logger.error("LiteLLM request failed for bot_id=#{bot.id}: #{e.class}")
      nil
    end
    ...
    def handle_failure(bot, channel, identity_user, _reason)
      ...
      Rails.logger.error("Failed to report bot error in channel #{channel&.id}: #{e.class}")
    end
    ...
    def broadcast_typing(channel, identity_user, typing)
      ...
      Rails.logger.debug("Failed to broadcast typing for bot_identity_id=#{identity_user.id}: #{e.class}")
    end
  end
end
